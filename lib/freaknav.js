;(function(wdw, doc) {
    ...
    Plugin.prototype = {
        ...
        findHotkeyItem: function(keycode){
            var i = this.navElements.length,
                hotkeyElement = this.currentObj;

            while(i--) {
                if(this.navElements[i].hasAttribute('data-hotkey') && this.navElements[i].getAttribute('data-hotkey') === keyMap[keycode]){
                    hotkeyElement = this.navElements[i];
                    break;
                }
            }
